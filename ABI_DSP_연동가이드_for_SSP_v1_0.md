# ABI DSP 연동 가이드 for SSP v1.0
***
- [1. ABI RTB Partner?](#1-ABI-rtb-partner)
    * [1.1. 소개](#11-소개)
    * [1.2. 지원 형식 및 요청](#12-지원-형식-및-요청)
        + [1.3.1. ABI RTB Partner 지원 소재 종류](#131-ABI-rtb-partner-지원-소재-종류)
        + [1.2.2. 전송](#122-전송)
        + [1.2.3. 헤더](#123-헤더)
        + [1.2.4. 데이터 인코딩](#124-데이터-인코딩)
...
# 1. ABI RTB Partner?
## 1.1. 소개
> ABI RTB Partner 는 광고 구매사와 퍼블리셔 인벤토리 판매자 간 실시간 자동 입찰을 진행하는 광고 경매 시스템 입니다. OpenRTB Specification version 2.3 ~ 2.6와 Video Ad Serving Template (VAST) 3.0 을 기반으로 설계되었으며, 모든 스펙 지원이 아닌 제한된 범위의 Object만이 구현되어 있습니다.

## 1.2. 지원 형식 및 요청
### 1.2.1. ABI RTB Partner 지원 소재 종류
- 디스플레이 배너: ABI 에서 구현된 소재를 HTML 형태로 반환
...
| pos       | integer        |          | 광고 위치 코드 [(Ad Position 표 참조)](#212-ad-position)              |
| btype     | array(integer) |          | 차단 할 배너 타입 코드 [(Banner Ad Types 표 참조)](#211-banner-ad-types) |
...
## 2.10. Imp > PMP
> Private Marketplace(PMP) 거래에 대한 정보를 담고 있습니다. 특정 구매자와 판매자 간의 사전 합의된 거래 조건을 정의합니다.

| Attribute       | Type          | Required | Comment                                                        |
|-----------------|---------------|----------|----------------------------------------------------------------|
| private_auction | integer       |          | 프라이빗 옥션 여부 플래그. 0 : 모든 입찰 허용, 1 : deals에 명시된 seat만 입찰 허용 |
| deals           | array(object) |          | Deal Object 리스트. 아래 Deal Object 표 참조                         |

### 2.10.1. Imp > PMP > Deal
> PMP 거래의 개별 Deal 정보를 나타냅니다.

| Attribute   | Type          | Required | Comment                                                           |
|-------------|---------------|----------|-------------------------------------------------------------------|
| id          | string        | O        | Deal의 고유 ID                                                       |
| bidfloor    | float         |          | Deal의 최저 CPM 입찰 단가                                                |
| bidfloorcur | string        |          | bidfloor의 통화 단위. ISO-4217 참조 (예: 'USD', 'KRW')                    |
| at          | integer       |          | 옥션 타입 (1: 1st price, 2: 2nd price, 3: Deal에 명시된 고정 가격)           |
| wseat       | array(string) |          | 해당 Deal에 입찰할 수 있는 seat ID 리스트                                    |
| wadomain    | array(string) |          | 해당 Deal에 입찰할 수 있는 광고주 도메인 리스트                                   |
